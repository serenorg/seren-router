...
pub struct RoutingPolicy {
    routes: Arc<HashMap<String, Vec<RouteDescriptor>>>,
...
struct RoutingState {
    shares: HashMap<String, ShareTracker>,
    rng: StdRng,
}
...
impl RoutingPolicy {
    ...
    fn from_registry_with_rng(
        ...
        let mut routes = HashMap::<String, Vec<RouteDescriptor>>::new();
    ...
    pub fn route(&self, request: &mut Value) -> Result<RouteDecision, RouteRequestError> {
        let parsed = parse_request(request)?;
        consume_routing_sort(request);
        let descriptors = self
            .routes
            .get(&parsed.canonical_model)
        ...
        let candidates: Vec<_> = descriptors
            ...
            .map(|route| {
                Candidate::new(
                    ...
                    route.priority,
                    self.measurements
                        .get(&route.provider_id, &parsed.canonical_model),
                    true,
                )
        ...
        let mut recent_share = state
            .shares
            .remove(&parsed.canonical_model)
            .unwrap_or_else(|| {
        ...
        let Some(selected) = select_route(
            ...
            state
                .shares
                .insert(parsed.canonical_model.clone(), recent_share);
...
mod tests {
    ...
    #[test]
    fn request_routing_uses_live_measurements_and_consumes_only_sort() {
        ...
        let mut request = json!({
            ...
            "provider": {"sort": "throughput", "ignore": ["blocked"]},
            "reasoning": {"effort": "high"}
        });

        let decision = routing.route(&mut request).unwrap();
        ...
        assert_eq!(request["provider"], json!({"ignore": ["blocked"]}));
    ...
    async fn unknown_models_fail_locally_with_json_not_found() {
        ...
        let response = routing.route(&mut request).unwrap_err().into_response();
